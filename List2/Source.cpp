...
class List
{
	...
	class BaseIterator
	{
	protected://Открываем прямой доступ к перменной Temp из дочерних классов
		Element* Temp;
	public:
...
void List<T>::insert(int index, T Data)
{
	...
	//неважно с какой стороны добрались до нужного элемента
	//процедура добавления нового элемента будеь идентичной
	/*Element* New = new Element(Data);
	New->pNext = Temp;
...
template<typename T>
void List<T>::reverse_print()const// выводит список задом на перед
{
	for (Element* Temp = Tail; Temp; Temp = Temp->pPrev)
		cout << Temp->pPrev << tab << Temp << tab << Temp->Data <<
		tab << Temp->pNext << endl;
	cout << "Количество элеметов списка: " << size << endl;
}
